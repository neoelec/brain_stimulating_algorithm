...
{
    ssize_t i;
    struct lldq __list, *list = &__list;
    struct dbll_node *node;
    struct lldq_entry *entry;
    ssize_t count;

    lldq_initialize(list);

    /* 노드 5개 추가 */
    for (i = 0; i < 5; i++) {
        entry = lldq_create_entry(i);
        lldq_push_rear(list, &entry->node);
    }

    /* 뎈 출력 */
    i = 0;
    lldq_forward(list, lldq_print_entry, &i);

    /* 뎈의 세 번째 노드 뒤에 새 노드 삽입 */
    printf("\nInserting 3000 After [2]...\n\n");
    entry = lldq_create_entry(3000);
    lldq_insert(list, 3, &entry->node);
    ...
    /* 모든 노드를 메모리에서 제거 */
    printf("\nDestroyihng List...\n");
    while (!lldq_is_empty(list)) {
        lldq_pop_rear(list, &node);
        free(container_of(node, struct lldq_entry, node));
    }
